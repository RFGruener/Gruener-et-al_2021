...
Ki-67 staining is not available, so we cannot do the traditional Lum A and Lum B groups.  
...
aov_preds.subtype %>% 
  filter(term == ".$PAM50") %>%
  ungroup() %>% 
  mutate(p.bonf_correct = p.adjust(p.value, method = "bonferroni")) %>% 
  arrange(p.bonf_correct) %>% 
  mutate(log_p.correct = log10(p.bonf_correct)) %>% 
  ggplot() +
...
ER_t.tst_preds <- preds.subtype %>% 
  group_by(Drug) %>% 
  drop_na(ER_CALL) %>% 
  mutate(ER_CALL = if_else(ER_CALL == 0, "Negative", "Positive")) %>%
  do(tidy(t.test(PSS~ER_CALL, data = .))) %>% 
  mutate(bonf_p.value = p.adjust(p.value)) %>% 
...
ESR1_PSS <- preds.subtype %>% 
  filter(Drug == "tamoxifen") %>% 
  drop_na(ESR1_mRNA) %$%
  cor.test(PSS, ESR1_mRNA, method = "spearman")

preds.subtype %>% 
  filter(Drug == "tamoxifen") %>% 
  ggplot(aes(x = ESR1_mRNA, y = PSS)) + 
    geom_point() +
    geom_smooth(method = "lm") + 
    annotate("text", x = 50000, y = 14.5, label = paste("p-value < 2.2 x 10^-16,  rho =", ESR1_PSS$estimate)) + 
    labs(title = "Imputed Tamoxifen Response Correlates with ESR1 Expression in TCGA BRCA Patients", 
         x = "ESR1 mRNA Levels", 
         y = "Tamoxifen Imputed Sensitivity Score (ISS)") 
...
HER2_t.tst_preds <- preds.subtype %>% 
  group_by(Drug) %>% 
  drop_na(HER2_PATH_CALL_1) %>% 
  filter(HER2_PATH_CALL_1 %in% c("negative", "positive")) %>%
  do(tidy(t.test(PSS~HER2_PATH_CALL_1, data = .))) %>% 
  mutate(bonf_p.value = p.adjust(p.value)) %>% 
  arrange(bonf_p.value) %>% 
  filter(estimate > 0)
