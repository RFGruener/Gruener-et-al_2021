...
#download TCGAbiolinks if needed
if(!requireNamespace("TCGAbiolinks", quietly = TRUE))
    BiocManager::install("TCGAbiolinks")

#download a slew of other packages if needed
req_packages <- c("broom", "grid", "ggrepel", "lsr", "svglite")
needed_packs <- req_packages[!(req_packages %in% installed.packages())]
if(length(needed_packs)) install.packages(needed_packs)
...
CTRP_cv %>% ggplot(aes(x = Spearman_Correlation)) + 
  geom_histogram() + 
...
BRCA_preds <- BRCA_preds %>% 
  ...
  filter(!Drug %in% bad_cv_drugs) #filter out drugs with poor CV results
...
ggsave(filename = "./Figures/Figure2a.svg", plot = fig2a)
...
#Note:  
  #3-Cl-AHPC: Unclear, one publication claimed that it doesn't activate RAR and works through AKT (PMID: 14981538) while a much more recent publication says that it does work through RAR (PMID: 30370662), while the database claims it is an orphan nuclear receptor agonist
...
#For the table in the text, I'm going to include only the top 5% MOA, but I'm going to also include any compound that fits in this MOA that appears in the top 10%. Looking at the data, several inhibitors are in the top 10% that match the top 5% and as such it seems like most of the top compounds have pretty consistent mechanisms of actions.  I'll make a table for the actual text in excel using the Supplementary data and some filters, but here is roughly what I'll be looking for:

e_10_cutoff <- sort(abs(t.tst_preds.subtype$estimate), decreasing = T)[55] 
p_10_cutoff <- sort(abs(t.tst_preds.subtype$bonf_p.value), decreasing = F)[55]
...
e_5_cutoff <- sort(abs(t.tst_preds.subtype$estimate), decreasing = T)[28] 
p_5_cutoff <- sort(abs(t.tst_preds.subtype$bonf_p.value), decreasing = F)[28]
...
top_TNBC_drugs <- top_targets_t.tst %>% 
  ...
  ungroup() %>% 
  filter(estimate >= e_5_cutoff,
        bonf_p.value <= p_5_cutoff) %>%
  arrange(bonf_p.value) %>% 
...
targets_t.test %>% 
  dplyr::select(Drug, "Pubchem ID" = `Harmonized ID (Pubchem ID)`, "Effect Size" = estimate, "Bonferroni Adjusted p-value" = bonf_p.value, 
         MOA, Targets, `Clinical Phase`, `Source for Clinical Information`, "N Drugs with same MOA" = Total_MOA_Count) %>% 
...
ggsave(filename = "./Figures/Figure2b.svg", plot = azd_clin)
...
ggsave(filename = "./Figures/Figure2c.svg", plot = azd_pam)
```
...
tam_clin
ggsave(filename = "./Figures/Supplement_2a.svg", plot = tam_clin)

tam_pam <- tam_pam + 
  labs(title = "Tamoxifen's Imputed Sensitivity in TCGA BRCA Cohort by PAM50 Subtype")
tam_pam
ggsave(filename = "./Figures/Supplement_2b.svg", plot = tam_pam)
