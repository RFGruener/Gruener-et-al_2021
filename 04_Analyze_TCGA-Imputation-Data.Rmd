...
effect_cutoff <- sort(abs(t.tst_preds.subtype$estimate), decreasing = T)[28] 
p_val_cutoff <- sort(abs(t.tst_preds.subtype$bonf_p.value), decreasing = F)[28]

targets_t.test %>% filter(estimate >= effect_cutoff,
         bonf_p.value <= p_val_cutoff) %>% count(MOA, Total_MOA_Count, sort = T)
...
write_csv(top_targets_t.tst, path = "./DataOut/Top_compounds.csv")
