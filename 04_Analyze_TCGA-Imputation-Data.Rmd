...
sup1
ggsave(plot = sup1, device = "eps", filename = "supplemental_figure_1.eps")
...
write_csv(preds.subtype, file = "Supplemental_table1.csv")
...
dplyr::filter(ER_t.tst_preds, estimate > 0)
#ER positivity associates with Imputed Tamoxifen Response, it is the 3th most significant result
```


```{r}

lum_t.tst_preds <- preds.subtype %>%
  filter(Clin_Subtype %in% c("ER/PR+", "HER2+")) %>% 
  group_by(Drug) %>% 
  do(tidy(t.test(PSS~Clin_Subtype, data = .))) %>% 
  ungroup() %>% 
  mutate(bonf_p.value = p.adjust(p.value, method = "bonferroni")) %>% 
  mutate(log10_p.correct = -log10(bonf_p.value)) %>% 
  arrange(bonf_p.value) 
...
lum_t.tst_preds %>% 
  ...
  ggplot(mapping = aes(x = estimate, y = log10_p.correct, label = Drug)) + 
    geom_point(data = lum_t.tst_preds) +
...
lum_t.tst_preds <- preds.subtype %>%
  filter(Clin_Subtype %in% c("ER/PR+", "TNBC")) %>% 
  group_by(Drug) %>% 
  do(tidy(t.test(PSS~Clin_Subtype, data = .))) %>% 
  ungroup() %>% 
  mutate(bonf_p.value = p.adjust(p.value, method = "bonferroni")) %>% 
...
lum_t.tst_preds <- preds.subtype %>%
  filter(Clin_Subtype %in% c("ER/PR+", "HER2+", "TNBC")) %>% 
  mutate(luminal = grepl("ER/PR+", x = .$Clin_Subtype)) %>% 
  group_by(Drug) %>% 
  do(tidy(t.test(PSS~luminal, data = .))) %>% 
  ungroup() %>% 
  mutate(bonf_p.value = p.adjust(p.value, method = "bonferroni")) %>% 
  mutate(log10_p.correct = -log10(bonf_p.value)) %>% 
  arrange(bonf_p.value)
...
lum_t.tst_preds <- preds.subtype %>%
  filter(PAM50 %in% c("LumA", "Basal")) %>% 
  group_by(Drug) %>% 
  do(tidy(t.test(PSS~PAM50, data = .))) %>% 
  ungroup() %>% 
  mutate(bonf_p.value = p.adjust(p.value, method = "bonferroni")) %>% 
  mutate(log10_p.correct = -log10(bonf_p.value)) %>% 
  arrange(bonf_p.value)
...
lum_t.tst_preds %>% 
  ...
  ggplot(mapping = aes(x = estimate, y = log10_p.correct, label = Drug)) + 
    ...
    labs(y = "-log10(adjusted p-value)", 
         x = "Mean Difference (Imputed Effect RPBC - TNBC)", 
         caption = "Drugs More Effective in Basal                                                                                                            Drugs More Effective in Luminal") + 
    theme(plot.title = element_text(hjust = 0.5))
...
lum_t.tst_preds <- preds.subtype %>%
  filter(Clin_Subtype %in% c("ER/PR+", "TNBC")) %>% 
  mutate(Clin_Subtype = factor(Clin_Subtype, levels = c("TNBC", "ER/PR+"))) %>% 
  group_by(Drug) %>% 
  do(tidy(t.test(PSS~Clin_Subtype, data = .))) %>% 
  ungroup() %>% 
  mutate(bonf_p.value = p.adjust(p.value, method = "bonferroni")) %>% 
...
e_10_cutoff <- sort(lum_t.tst_preds$estimate[lum_t.tst_preds$estimate > 0], 
                    decreasing = T)[round(sum(lum_t.tst_preds$estimate > 0)*0.1, digits = 0)] 
p_10_cutoff <- sort(lum_t.tst_preds$bonf_p.value[lum_t.tst_preds$estimate > 0], 
                    decreasing = F)[round(sum(lum_t.tst_preds$estimate > 0)*0.1, digits = 0)]

top_lum_drugs <- lum_t.tst_preds %>% 
  filter(estimate > 0, 
...
lum_t.tst_preds %>% 
  ...
  arrange(desc(log10_p.correct)) %>% 
  dplyr::slice(1:11) %>% 
  ggplot(mapping = aes(x = 1, y = log10_p.correct, label = Drug)) + 
    geom_point(data = filter(lum_t.tst_preds, estimate > 0)) +
...
lum_t.tst_preds %>% 
  filter(estimate > 0) %>% 
  mutate(label = if_else(bonf_p.value < p_10_cutoff, Drug, ""),
         Signific = if_else(bonf_p.value < p_10_cutoff, "Lead", "")) %>% 
  ggplot(mapping = aes(x = 1, y = log10_p.correct, label = label)) + 
  geom_point(aes(color = Signific)) + 
  scale_color_manual(values = c("black", "blue")) + 
  geom_text_repel(nudge_x = 0.01, force = 3) + 
  scale_x_continuous(name = "Drugs with Predicted Preferential \nAcitivity in HR+ Breast Cancer", 
                     limits = c(1, 1.08), breaks = NULL) + 
    labs(y = "-log10(adjusted p-value)") +
   theme_classic() +
...
t.tst_preds.subtype %>% 
  arrange(desc(log10_p.correct)) %>% 
  dplyr::slice(1:3) %>% 
  bind_rows(slice(arrange(filter(t.tst_preds.subtype, estimate < 0),
                                    desc(log10_p.correct)),
                            1:2)) %>% 
  ggplot(mapping = aes(x = estimate, y = log10_p.correct, label = Drug)) + 
    geom_point(data = t.tst_preds.subtype) +
    geom_point(color = "red", size = 2.2) +
    geom_point(data = slice(arrange(filter(t.tst_preds.subtype, estimate < 0),
                                    desc(log10_p.correct)),
                            1:2), 
               color = "blue", size = 2.2, mapping = aes(label = Drug)) +
    geom_text_repel(nudge_y = 3) + 
...
fig2a <- t.tst_preds.subtype %>% 
  ...
  ggplot(mapping = aes(x = estimate, y = log10_p.correct, label = Drug)) + 
    ...
    labs(y = "-log10(adjusted p-value)", 
         x = "Mean Difference (Imputed Effect RPBC - TNBC)", 
         caption = c("Drugs More\nEffective in TNBC", " Drugs More\nEffective in RPBC")) + 
    theme(plot.caption = element_text(hjust=c(1, -0.1))) 
    
fig2a

ggsave(filename = "./Figures/Figure2a.TIFF", device = "tiff", plot = fig2a, height = 5.5, width = 7)
...
e_10_cutoff <- sort(t.tst_preds.subtype$estimate, 
                    decreasing = T)[round(sum(t.tst_preds.subtype$estimate > 0)*0.1, digits = 0)] 
p_10_cutoff <- sort(abs(t.tst_preds.subtype$bonf_p.value), 
                    decreasing = F)[round(sum(t.tst_preds.subtype$estimate > 0)*0.1, digits = 0)]
...
Sensitivity_grapher(drug = "leptomycin B", comparison = "Clin_Subtype", group = "TNBC")
...
ggsave(filename = "./Figures/Figure2b.tiff", device = "tiff", plot = azd_clin)
...
ggsave(filename = "./Figures/Figure2c.tiff", device = "tiff", plot = azd_pam)
```
