...
Merge_preds_GDSC.IC50 %>% 
  ...
  ggplot(aes(x = `MK-1775`, y = LN_IC50)) + 
    ...
    labs(title = "Correlation of Imputed and Measured MK1775 Sensitivity in GDSC", 
...
Merge_preds_GDSC.IC50 %>% 
  ...
  ggplot(aes(x = `MK-1775`, y = LN_IC50)) + 
    ...
    geom_smooth(method = "lm") + 
    ggpubr::stat_cor(method = "spearman") + 
    labs(title = "Correlation of Imputed and Actual MK1775 Sensitivity in GDSC Breast CCLs",
         x = "Imputed Sensitivty Score", 
         y = "Measured ln(IC50) values from GDSC")
...
Merge_preds_GDSC.IC50 %>% 
  ...
  ggplot(aes(x = `MK-1775`, y = LN_IC50)) + 
    ...
    geom_smooth(method = "lm") + 
    ggpubr::stat_cor(method = "spearman") + 
    labs(title = "Correlation of Imputed and Measured MK1775 Sensitivity \n in Cell Lines Unique to GDSC", 
         x = "Imputed Sensitivty Score", 
         y = "Measured ln(IC50) values from GDSC")
...
#graph measured and predicted response in all GSDC cell lines that overlap between GDSC and CTRP to GDSC. Probably should have made a graphing function, but oh well. 
Merge_preds_GDSC.IC50 %>% 
...
#not surprising, these graphs look essentially the same as the overall and unique GDSC/ISS graphs
...
overlapping_CCLs %>% 
  ggplot(aes(x = Avg_AUC, y = LN_IC50)) + 
    ...
    geom_smooth(method = "lm") + 
    ggpubr::stat_cor(method = "spearman") + 
    labs(title = "Correlation of Mesured MK1775 Sensitivity from GDSC and CTRP", 
...
overlapping_CCLs %>% 
  ...
  ggplot(aes(x = Avg_AUC, y = LN_IC50)) + 
    ...
    geom_smooth(method = "lm") + 
    ggpubr::stat_cor(method = "spearman") + 
    labs(title = "Correlation of Mesured MK1775 Sensitivity from GDSC and CTRP in BRCA CCLs", 
         x = "Measured AUC Values from CTRP", 
         y = "Measured ln(IC50) values from GDSC")
...
```{r}
CTRP_aucs %>%
filter(cpd_name == "olaparib") %>% 
left_join(., ccls, by = c("CCL_Name" = "Study Cell Line Name")) %>%
distinct() %>% 
drop_na(`Harmonized Identifier`) %>% 
filter(`Cancer Statistics 2017 Sub-Category` == "Breast", 
       apparent_ec50_umol < 150) %>% 
  ggplot(aes(x = apparent_ec50_umol)) + 
    geom_histogram() + 
...
mutate(rank = dense_rank(apparent_ec50_umol)) %>% 
  ggplot(aes(x = rank, y = apparent_ec50_umol)) + 
    geom_point() + 
